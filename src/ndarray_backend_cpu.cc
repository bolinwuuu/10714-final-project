...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  while (true) {
      ...
      for (size_t d = indices.size() - 1; d >= 0; d--) {
          indices[d] += 1;
...
template <typename Func>
void EwiseOp(const AlignedArray& a, AlignedArray* out, Func func) {
  for (size_t i = 0; i < a.size; i++) {
...
void ScalarMul(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [](scalar_t x, scalar_t val) { return x * val; });
}
...
void ScalarPower(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [](scalar_t x, scalar_t val) { return std::pow(x, val); });
}
...
void ScalarMaximum(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [](scalar_t x, scalar_t val) { return std::max(x, val); });
}
...
void ScalarEq(const AlignedArray& a, scalar_t val, AlignedArray* out) {
  ScalarOp(a, val, out, [](scalar_t x, scalar_t val) { return x == val ? 1.0f : 0.0f; });
}
...
void EwiseLog(const AlignedArray& a, AlignedArray* out) {
  EwiseOp(a, out, [](scalar_t x) { return std::log(x); });
}

void EwiseExp(const AlignedArray& a, AlignedArray* out) {
  EwiseOp(a, out, [](scalar_t x) { return std::exp(x); });
}

void EwiseTanh(const AlignedArray& a, AlignedArray* out) {
  EwiseOp(a, out, [](scalar_t x) { return std::tanh(x); });
}
  ...
  for (size_t i = 0; i < m; i++) {
    for (size_t j = 0; j < p; j++) {
      ...
      for (size_t k = 0; k < n; k++) {
        curr_sum += a.ptr[i * n + k] * b.ptr[k * p + j];
      }
      out->ptr[i * p + j] = curr_sum;
    }
  }
  /// END SOLUTION
}
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < a.size; i += reduce_size) {
    scalar_t curr_max = std::numeric_limits<scalar_t>::lowest();
    for (size_t j = i; j < i + reduce_size; j++) {
      curr_max = std::max(curr_max, a.ptr[j]);
    }
    out->ptr[cnt++] = curr_max;
  }
  /// END SOLUTION
}
