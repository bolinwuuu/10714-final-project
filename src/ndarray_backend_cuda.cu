...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for (int d = shape.size - 1; d >= 0; d--) {
      size_t dim_idx = out_idx % shape.data[d];
      in_idx += dim_idx * strides.data[d];
      out_idx /= shape.data[d];
    }
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the ewise setitem opeation.
   * 
   * Args:
   *   a: CUDA pointer to a compact array
   *   out: CUDA point to out non-compact array
   *   size: size of out array
   *   shape: vector of shapes of a and out arrays (of type CudaVec, for past passing to CUDA kernel)
   *   strides: vector of strides of out array
   *   offset: offset of out array
...
__global__ void ScalarSetitemKernel(scalar_t val, scalar_t* out, size_t size, CudaVec shape,
                              CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the scalar setitem opeation.
   * 
   * Args:
   *   a: CUDA pointer to a compact array
   *   out: CUDA point to out non-compact array
...
// __global__ void EwiseOpKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size, Op operation) {
//   size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
// template<typename Op>
// __global__ void ScalarOpKernel(const scalar_t* a, const scalar_t val, scalar_t* out, size_t size, Op operation) {
//   size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
// template<typename Op>
// __global__ void EwiseOpKernel(const scalar_t* a, scalar_t* out, size_t size, Op operation) {
//   size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, 
   ...
   *   P: columns of b / out
   */
  int xbase = blockIdx.y * TILE + threadIdx.y;
  int ybase = blockIdx.x * TILE + threadIdx.x;
  scalar_t curr_sum = 0;

  for (int tile_idx = 0; tile_idx < (N + TILE - 1) / TILE; tile_idx++) {
    __shared__ scalar_t tile_a[TILE][TILE];
    __shared__ scalar_t tile_b[TILE][TILE];
    
    if (xbase < M && tile_idx * TILE + threadIdx.x < N) {
      tile_a[threadIdx.y][threadIdx.x] = a[xbase * N + tile_idx * TILE + threadIdx.x];
    } else {
      tile_a[threadIdx.y][threadIdx.x] = 0;
    }
    if (ybase < P && tile_idx * TILE + threadIdx.y < N) {
      tile_b[threadIdx.y][threadIdx.x] = b[(tile_idx * TILE + threadIdx.y) * P + ybase];
    } else {
    ...
    for (int k = 0; k < TILE; k++) {
      curr_sum += tile_a[threadIdx.y][k] * tile_b[k][threadIdx.x];
    }
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t curr_max = a[gid * reduce_size];
    for (size_t i = 1; i < reduce_size; i++) {
      curr_max = max(curr_max, a[gid * reduce_size + i]);
    }
    out[gid] = curr_max;
  }
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    scalar_t curr_sum = a[gid * reduce_size];
    for (size_t i = 1; i < reduce_size; i++) {
      curr_sum += a[gid * reduce_size + i];
    }
    out[gid] = curr_sum;
  }
}
