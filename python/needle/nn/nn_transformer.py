...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        logits = self.matmul(q, k) / np.sqrt(q_dim)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        Q_prime = self.q_projection(q_norm)
        K_prime = self.k_projection(k_norm)
        V_prime = self.v_projection(v_norm)
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mlp_dropout1 = Dropout(dropout)
        self.mlp_dropout2 = Dropout(dropout)
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        # attn_output = self.attention(x, k=x, v=x)
        attn_output = self.attention(x)
        attn_output = self.attn_dropout(attn_output)
        x = x + attn_output

        mlp_input = self.ln(x.reshape((batch_size * seq_len, x_dim)))
        mlp_output = self.linear1(mlp_input)
        mlp_output = self.relu(mlp_output)
        mlp_output = self.mlp_dropout1(mlp_output)
        mlp_output = self.linear2(mlp_output)
        mlp_output = self.mlp_dropout2(mlp_output)
        mlp_output = mlp_output.reshape(x.shape)
        x = x + mlp_output
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def forward(
        ...
        positions = np.broadcast_to(positions.reshape((-1, 1)),(seq_len, batch_size))

        positions = Tensor(positions, device=self.device, dtype=self.dtype).transpose((0,1))
