...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        new_array = self.make(self._shape, 
                              strides=self._strides, 
                              device=self._device,
                              handle=self._handle)
        new_array._shape = tuple(new_shape)
        new_array._strides = NDArray.compact_strides(new_array._shape)
        return new_array
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        new_array = self.make(self._shape, 
                              ...
                              device=self._device,
                              handle=self._handle)
        new_array._shape = tuple([new_array._shape[ax] for ax in new_axes])
        new_array._strides = tuple([new_array._strides[ax] for ax in new_axes])
        return new_array
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert len(self._shape) <= len(new_shape), "invalid new shape"
        for i in range(1, len(self._shape) + 1):
            assert self._shape[-i] == 1 or self._shape[-i] == new_shape[-i], "invalid new shape"
        ...
        new_array._strides = tuple(new_strides)
        return new_array
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        new_array._offset = self._offset
        new_strides = list(self._strides)
        new_shape = list(self._shape)
        for i, sl in enumerate(idxs):
            new_array._offset += sl.start * self._strides[i]
            new_strides[i] *= sl.step
            new_shape[i] = math.ceil((sl.stop - sl.start) / sl.step)
        new_array._strides = tuple(new_strides)
        new_array._shape = tuple(new_shape)
    ...
    def flip(self, axes):
        ...
        new_offset = 0
        for axis in axes:
            new_offset += (self.shape[axis] - 1) * new_strides[axis]
            new_strides[axis] = -new_strides[axis]
        new_strides = tuple(new_strides)

        return NDArray.make(self.shape, strides=new_strides, device=self._device, 
    ...
    def pad(self, axes):
        ...
        new_shape = tuple(
            left + size + right
            for size, (left, right) in zip(self.shape, axes)
        )
        res = full(new_shape, 0, dtype=self.dtype, device=self.device)
        self_slice = tuple(
