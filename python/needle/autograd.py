...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
        # v_i = Tensor(sum(node_to_output_grads_list[i]), dtype=out_grad.dtype)
        v_i = sum_node_list(node_to_output_grads_list[i])
        i.grad = v_i

        if i.op:
            input_grad = i.op.gradient(v_i, i)

            input_grad = i.op.gradient_as_tuple(v_i, i)
            for k, k_grad in zip(i.inputs, input_grad):
                if k in node_to_output_grads_list:
                    node_to_output_grads_list[k].append(k_grad)
                else:
                    node_to_output_grads_list[k] = [k_grad]
    ### END YOUR SOLUTION
